...
def generate_rss_xml(cleaned_products):
  """Génère un flux XML Google Shopping RSS 2.0 complet et optimisé pour supporter un grand nombre de déclinaisons sans surcharge mémoire."""
  ...
  for prod in cleaned_products:
    ...
    buffer.write(
        '        <description>'
        f'{cdata(prod.get("description"))}'
        '</description>\n'
    )
    buffer.write(f'        <link>{cdata(prod.get("link"))}</link>\n')
    buffer.write(
        '        <g:image_link>'
        f'{cdata(prod.get("image_link"))}'
        '</g:image_link>\n'
    )
    ...
    if availability == "backorder" and prod.get("availability_date"):
      buffer.write(
          '        <g:availability_date>'
          f'{prod.get("availability_date")}'
          '</g:availability_date>\n'
      )

    buffer.write(
        f'        <g:price>{prod.get("price", "0 EUR")}</g:price>\n'
    )
    buffer.write(
        f'        <g:gender>{prod.get("gender", "unisex")}</g:gender>\n'
    )
    buffer.write(
        f'        <g:age_group>{prod.get("age_group", "adult")}</g:age_group>\n'
    )
    buffer.write(
        '        <g:identifier_exists>'
        f'{prod.get("identifier_exists", "no")}'
        '</g:identifier_exists>\n'
    )
...
def upload_to_drive(
    xml_content, folder_id, target_filename="Shopping Graph_feed.xml"
):
  ...
  query = (
      f"'{folder_id}' in parents and name = '{target_filename}' and trashed ="
      " false"
  )
  response = (
      service.files()
  ...
  logger.info(
      f"Fichier {target_filename} mis à jour avec succès sur Google Drive."
  )
...
def main():
  ...
  # Initialisation du client API WooCommerce
  wcapi = API(
      url=woo_url,
      consumer_key=woo_ck,
  ...
  if target_lang and target_file:
    logger.info(
        f"=== Début de l'extraction Matrix ({target_lang}) -> {target_file} ==="
    )
    try:
      raw_products = fetch_all_products_with_variations(
          wcapi, lang=target_lang
      )
      cleaned_products = [
          process_product_item(item, lang=target_lang) for item in raw_products
      ]

      # Log de contrôle des URLs générées
      if cleaned_products:
  ...
  for lang_code, filename in target_languages.items():
    logger.info(
        f"=== Début de l'extraction pour la langue : {lang_code.upper()} ==="
    )
    try:
      ...
      logger.error(
          "Erreur lors du traitement de la langue"
          f" {lang_code.upper()} : {e}"
      )
