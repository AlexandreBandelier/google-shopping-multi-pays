...
# OPTIMISATION 1 : Pré-compilation de la Regex de nettoyage HTML hors de la boucle
...
def format_iso_timestamp(date_string):
    ...
    """
    if not date_string:
        # OPTIMISATION 1 (suite) : Remplacement de datetime.utcnow() (déprécié)
...
def clean_cdata(text):
    """
    OPTIMISATION 2 : Échappe la séquence ']]>' pour éviter de casser la structure XML.
    """
    if not text:
        return ""
...
def generate_reviews_xml(reviews, woo_url):
    """
    Génère le flux XML au format officiel Google Product Reviews (v2.3) optimisé.
    """
...
for rev in reviews:
        ...
        # 1. Date au format ISO 8601 strict (résout "Missing timestamp")
        timestamp = format_iso_timestamp(rev.get('date_created', ''))
        
        # 2. Contenu texte (résout "Unsupported comment format")
        ...
        # FIX: review_url avec l'attribut type="singleton"
        xml_parts.append('      <review_urls>\n')
        xml_parts.append(f'        <review_url type="singleton"><![CDATA[{review_url}]]></review_url>\n')
        xml_parts.append('      </review_urls>\n')
        ...
        # FIX: product_ids utilisant mpns / product_raw_id et product_url
        ...
        if product_sku:
            ...
            xml_parts.append('            </mpns>\n')
        xml_parts.append('          </product_ids>\n')
        xml_parts.append(f'          <product_url><![CDATA[{product_permalink}]]></product_url>\n')
        xml_parts.append('        </product>\n')
        xml_parts.append('      </products>\n')
